...
from dilate.dilate_loss import dilate_loss
...
def evaluate(data, X, Y, model, evaluateSDTW, evaluateL2, evaluateL1, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        dilate_full, dilate_shape, dilate_temporal = dilate_loss(output, Y, alpha, gamma, device)
        total_dilate += dilate_full.item()
    ...
    return dilate, sdtw, rse, rae, correlation;

def train(data, X, Y, model, criterion, optim, batch_size, train_on_dilate):
    model.train();
    total_loss = 0;
    n_samples = 0;
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        if train_on_dilate:
            loss, loss_shape, loss_temporal = dilate_loss(output, Y, alpha, gamma, device)
        else:
...
if args.loss == 'rae':
    ...
    train_on_dilate = True
evaluateL2 = nn.MSELoss(size_average=False);
evaluateL1 = nn.L1Loss(size_average=False)
evaluateSDTW = SoftDTW()
if args.cuda:
    if not train_on_dilate:
...
try:
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        train_loss = train(Data, Data.train[0], Data.train[1], model, criterion, optim, args.batch_size, train_on_dilate)
        val_dilate, val_sdtw, val_rse, val_rae, val_corr = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateSDTW, evaluateL2, evaluateL1, args.batch_size);
        
        #Save the model, optim, history and val_history
        history.append(({args.loss:train_loss}, {'dilate' : val_dilate, 'sdtw' : val_sdtw, 'rae' : val_rae,'rse' : val_rse, 'corr' : val_corr}))
        checkpoint(os.path.join(args.save, 'model.pt'), model, optim.optimizer, history)
        #plot the statistics
        plot_separate(history, epoch, output_dir = args.save, plot_type='dilate')
        ...
        print('| end of epoch {:3d} | time: {:5.2f}s | train_loss {:5.4f} |  valid dilate {:5.4f} | valid sdtw {:5.4f} | valid rse {:5.4f} | valid rae {:5.4f} | valid corr  {:5.4f}'.format(epoch, (time.time() - epoch_start_time), train_loss,val_dilate, val_sdtw, val_rse, val_rae, val_corr))
#         print(' end of epoch')
        if epoch % 5 == 0:
            test_dilate, test_sdtw, test_rse, test_rae, test_corr  = evaluate(Data, Data.test[0], Data.test[1], model, evaluateSDTW, evaluateL2, evaluateL1, args.batch_size);
            test_stats = dict()
            test_stats = {'test_dilate':test_dilate, 'test_sdtw':test_sdtw, 'test_rae':test_rae,'test_rse':test_rse, 'test_corr': test_corr}
            test_stat_path = os.path.join(args.save, 'test_stat.pt')
            torch.save(test_stats,test_stat_path)
            print ("test dilate {:5.4f} | test sdtw {:5.4f} | test rse {:5.4f} | test rae {:5.4f} | test corr {:5.4f}".format(test_dilate, test_sdtw, test_rse, test_rae, test_corr))
